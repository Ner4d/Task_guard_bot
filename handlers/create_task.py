...
from keyboards.kb_common_cmd import kb_button_cancel, kb_button_skip
...
async def create_task_title_long(message: types.Message) -> None:
    keyboard = await kb_button_cancel()
    await message.answer(text='Слишком длинное название.'
                              '\nПопробуйте снова или', reply_markup=keyboard)
    await asyncio.sleep(5)
    await message.delete_reply_markup()


# Шаг 2-ой - skip
async def create_task_description_skip(query: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await query.message.edit_text(text='Отлично. Теперь необходимо назначить крайний срок'
