...
async def prepare_text_task(task: TaskModel) -> str:
    marks_for_status: dict[str, str] = {
        TaskModel.STATUSES.in_process: green_circle_emoji,
        TaskModel.STATUSES.completed: blue_circle_emoji,
        TaskModel.STATUSES.failed: red_circle_emoji,
        TaskModel.STATUSES.overtime: orange_circle_emoji
    }
    text_parts: list[str] = [f'Задача: {B_TEXT.format(task.title)}']
    if task.description:
